...
class Deck {
  ...
  shuffle() {
    return warDeck.cards.sort(function(a, b) {
...
let thisIsWar = () => {
  ...
  if (playerOneDeck.length < 4) {
    console.log(
      `Player One has fallen to ${playerOneDeck.length} cards and must accept defeat. PLAYER TWO WINS! *ENDGAME*`
    );
  } else if (playerTwoDeck.length < 4) {
    console.log(
      `Player Two has fallen to ${playerTwoDeck.length} cards and must accept defeat. PLAYER ONE WINS! *ENDGAME*`
    );
  } else {
    warCardsInPlay();
    warCardsInPlay();
    ...
    console.log(
      `Player One draws 3 cards facedown and plays a ${playerOnePile[0].rank} of ${playerOnePile[0].suit}`
    );
    console.log(
      `Player Two draws 3 cards facedown and plays a ${playerTwoPile[0].rank} of ${playerTwoPile[0].suit}`
    );
    evaluateWar();
  }
};
...
let evaluateWar = () => {
  if (playerOnePile[0].score > playerTwoPile[0].score) {
    ...
    console.log(
      `Player One WINS the WAR - card amount: ${playerOneDeck.length}`
    );
    console.log(`     Player Two - card amount: ${playerTwoDeck.length}`);
    determineWinner();
  } else if (playerOnePile[0].score < playerTwoPile[0].score) {
    ...
    console.log(
      `Player Two WINS the WAR - card amount ${playerTwoDeck.length}`
    );
    console.log(`     Player One - card amount ${playerOneDeck.length}`);
    determineWinner();
  } else {
